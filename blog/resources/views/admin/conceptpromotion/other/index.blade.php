...
<section class="content">
    ...
    @if(str_replace(' ', '-', $side->title) == $query)
    <a href="conceptpromotionother/create?concept={{ $side->title }}" class="btn btn-success">
    @endif
    @endforeach
                ...
                <div class="form-group col-md-12">
                    ...
                    <select class="form-control" name="conceptInfo">
                        ...
                        @foreach($sidebar as $side)
                            <option value="{{ $side->title }}"
                                @if($query==str_replace(' ', '-', $side->title)) selected @endif>{{ $side->title }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-sm-12">
                    <table id="myTable" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
                        ...
                        <tbody>
                        @if (isset($listconceptpromotionother) && count($listconceptpromotionother) >0)
                                ...
                                <tr role="row" class="odd">
                                    ...
                                    <td>
                                        <div class="btn-group" style="float: right;">
                                            <a href="{{ url('admin/conceptpromotionother')}}/{{ $ser->name }}/edit" class="btn btn-default bg-purple">

                                                <span>Edits</span>
                                            </a>
                                            <form action="{{ url('admin/conceptpromotionother')}}/{{ $ser->name }}" method="POST">
